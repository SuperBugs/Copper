...
//从服务器获取ChinaFragmentListView要展示的数据
public class ChinaTaskListViewDate {
    ...
    private void requestDate() {
        /**
         *拼装ChainListViewAddress：url：http://url/index.php?c=ChinaTaskListData&f=get
         */
        CommonConfig config = new CommonConfig();
        String MainListViewAddress = config.getUrlChinaTaskListViewDateFunction();
        String response = MyHttpUrlConnectionGet.sendHttpRequest(MainListViewAddress);
        if (response == null) {
        } else {
            try {
                JSONObject jsonObject = new JSONObject(response);
                JSONArray jsonList = jsonObject.getJSONArray("ChinaTaskListViewData");
