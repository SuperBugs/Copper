...
//从服务器获取chainFragmentListView要展示的数据
public class ChinaAppListViewDate {
    ...
    private void requestDate() {
        /**
         *拼装ChainListViewAddressurl：http://url/index.php?c=ChinaAppListData&f=get
         */
        CommonConfig config = new CommonConfig();
        String ChinaAppListViewAddress = config.getUrlChinaAppListViewDateFunction();

        String response = MyHttpUrlConnectionGet.sendHttpRequest(ChinaAppListViewAddress);
        if (response == null) {
        }else {
            try {
                JSONObject jsonObject = new JSONObject(response);
                JSONArray jsonList = jsonObject.getJSONArray("ChinaAppListViewData");
