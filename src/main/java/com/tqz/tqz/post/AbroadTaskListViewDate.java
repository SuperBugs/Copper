...
public class AbroadTaskListViewDate {
    ...
    private void requestDate() {
        /**
         *拼装ChainListViewAddress：http://url/index.php?c=
         *ChainListViewData&f=get
         */
        CommonConfig config = new CommonConfig();
        String AbroadTaskViewAddress = config.getUrlAbroadTaskListViewDateFunction();

        String response = MyHttpUrlConnectionGet.sendHttpRequest(AbroadTaskViewAddress);
        if (response == null) {
        } else {
            try {
                JSONObject jsonObject = new JSONObject(response);
                JSONArray jsonList = jsonObject.getJSONArray("AbroadTaskListViewData");
